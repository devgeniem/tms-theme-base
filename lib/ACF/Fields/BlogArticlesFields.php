...
/**
 * Class BlogArticlesFields
 *
 * @package TMS\Theme\Base\ACF\Fields
 */
class BlogArticlesFields extends \Geniem\ACF\Field\Group {
    ...
    public function __construct( $label = '', $key = null, $name = null ) {
        ...
        $this->strings = [
            ...
            'highlight_blog_article' => [
                'label'        => 'Korostettu blogiartikkeli',
                'instructions' => '',
            ],
            'article'                => [
                'label'        => 'Blogiartikkeli',
                'instructions' => 'Ylikirjoita blogiartikkelin ote. Tyhjäksi jättäminen käyttää oletusta.',
            ],
            'article_excerpt'        => [
                'label'        => 'Ote',
                'instructions' => '',
            ...
            'category'               => [
                'label'        => 'Kategoriat',
                'instructions' => 'Esitä blogiartikkeleja valituista kategorioista',
            ],
            'article_repeater'       => [
                'label'        => 'Blogiartikkelit',
                'instructions' => 'Valitse 4-12 blogiartikkelia',
     ...
     */
    protected function blog_article_item_field_group() : Field\Group {
        $group = ( new Field\Group( $this->strings['article']['label'] ) )
            ->hide_label()
            ->set_key( $this->get_key() . '_article_item' )
            ->set_name( 'article_item' );

        $blog_article_field = ( new Field\PostObject( $this->strings['article']['label'] ) )
            ->set_key( $this->get_key() . 'article' )
            ->set_name( 'article' )
        ...
        $group->add_fields( [ $blog_article_field, $excerpt_field ] );
    ...
    protected function sub_fields() : array {
        ...
        $blog_article_item_manual_field_group = $this->blog_article_item_field_group();

        $blog_articles_repeater_field = ( new Field\Repeater( $this->strings['article_repeater']['label'] ) )
            ->set_key( $this->get_key() . '_blog_article_repeater' )
        ...
        $blog_articles_repeater_field->add_field( $blog_article_item_manual_field_group );
        $blog_articles_repeater_field->add_conditional_logic( $rule_group_manual );

        return [
            $title_field,
            $description_field,
            $highlight_blog_article_field,
            $feed_type_field,
            $blog_articles_repeater_field,
            $category_field,
            $limit_field,
            $link_field,
            $display_image_field,
